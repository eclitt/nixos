...
let
  weatherScript = pkgs.writeShellScriptBin "weather" ''
    ...
    get_weather() {
        ${pkgs.curl}/bin/curl -s --connect-timeout 5 "wttr.in/?format=1"
    }
  ...
  programs.hyprlock = {
    ...
    settings = {
      ...
      label = [
        ...
        {
          text = "cmd[update:60000] ${weatherScript}/bin/weather"; # Используйте абсолютный путь
          color = "rgb(FFFFFF)";
          font_size = 25;
          font_family = "SF Pro Display Bold";
          position = "-10, 25";
          halign = "center";
          valign = "bottom"; # Исправлено: 'left' недопустимо. Используйте 'top', 'center', 'bottom' или 'none'.
          shadow_passes = 5;
          shadow_size = 10;
        }
      ];
